...
# Define the cities with their names and coordinates
city_mapping = {
...
# Create a Proj object for the UTM zone 10, WGS84
p = Proj(proj='utm', zone=10, ellps='WGS84', preserve_units=False)

# This will convert latitude and longitude to UTM coordinates
def to_utm(lat, lon):
    return p(lon, lat)

# Convert all cities to their UTM coordinates
for city in city_mapping.values():
    city[1] = to_utm(*city[1])

def create_route():
    # Create a route that always starts with Seattle (index 0)
    route = list(city_mapping.keys())
    route.remove(0)  # Remove Seattle
    random.shuffle(route)
    route.insert(0, 0)  # Insert Seattle at the start
    print(route)    
    return route
...
def main():
    random.seed(42)
    # The rest of the DEAP setup remains unchanged
...
# Update the plotting functions to use the city_mapping
...
if __name__ == "__main__":
    _, _, hof, best_initial, logbook = main()

    plot_route(best_initial)

    best_route = hof[0]
    # Ensure Seattle is at the start of the best route
    # if best_route[0] != 0:
    #     best_route.remove(0)
    #     best_route.insert(0, 0)
    #     print("Seattle was not at the start of the best route. Fixed.")
